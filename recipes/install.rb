...
when 'package'
  ...
  when 'debian'
    ...
    raise "installed_method package is not available for #{node['platform_family']} platform_family"
  end
else
  raise "invalid install_method #{node['thumbor']['install_method']}, valid are pip package"
end
